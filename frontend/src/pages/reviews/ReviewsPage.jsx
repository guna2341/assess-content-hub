    ...
    questions:
      [
        ...
        },
        {
          ...
          type: 'Short Answer',
          topic: 'variables',
          status: 'needs edit', // Changed to needs edit for review
          difficulty: 'Easy',
          correctAnswer: 'A symbol that represents a number that can change.',
          explanation: 'A variable is a placeholder for a value that can change, commonly represented by letters.',
          createdBy: 'Dr. Smith',
...
const questionsNeedingReview = contentUnits.flatMap(unit =>
  unit.questions
    ?.filter(question => question.status === 'pending' || question.status === 'needs edit')
    ?.map(question => ({
      ...question,
      contentUnit: {
        id: unit.id,
...
export function ReviewsPage() {
  ...
  // Get unique faculty members for filter
  const facultyMembers = [...new Set(questions.map(q => q.createdBy))];

  // Filter questions based on search and filters
  const filteredQuestions = questions.filter(question => {
    const matchesSearch = question.question.toLowerCase().includes(searchQuery.toLowerCase()) ||
      question.topic.toLowerCase().includes(searchQuery.toLowerCase()) ||
      question.contentUnit.code.toLowerCase().includes(searchQuery.toLowerCase());
    const matchesFaculty = facultyFilter === 'all' || question.createdBy === facultyFilter;
    const matchesDifficulty = difficultyFilter === 'all' || question.difficulty === difficultyFilter;
    const matchesType = typeFilter === 'all' || question.type === typeFilter;
    const matchesStatus = statusFilter === 'all' || question.status === statusFilter;

    return matchesSearch && matchesFaculty && matchesDifficulty && matchesType && matchesStatus;
  });

  // Calculate stats
  const totalQuestions = questions.length;
  const pendingQuestions = questions.filter(q => q.status === 'pending').length;
  const needsEditQuestions = questions.filter(q => q.status === 'needs edit').length;
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between pt-2 border-t">
                  ...
                  <div className="flex items-center gap-2">
                    <Button variant="outline" size="sm" onClick={() => alert(`Viewing details for question ${question.id}`)}>
                    ...
                    <Button
                      ...
                      onClick={() => handleReviewAction('edit', question.id)}
                    >
      ...
      {filteredQuestions.length === 0 && (
        <Card className="bg-gradient-card border-0 shadow-soft">
          <CardContent className="p-12 text-center">
            ...
            <p className="text-muted-foreground">
              {searchQuery || facultyFilter !== 'all' || difficultyFilter !== 'all' || typeFilter !== 'all' || statusFilter !== 'all'
